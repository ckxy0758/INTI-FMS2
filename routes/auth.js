...
// Configure the SMTP transport layer securely
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'courneyk8570@gmail.com',
    pass: '' // Secure App Password integration
  }
});

// Reusable function to dispatch emails asynchronously during lifecycle state changes
function sendEmailNotification(userEmail, title, message) {
  ...
  // Non-blocking execution: transporter.sendMail runs in the background. 
  // The server does NOT wait for the email to send before continuing code execution.
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) console.error("Email error:", error); // Logs error without crashing the server
...
router.post("/login", (req, res) => {
  ...
  db.query(sql, [email], (err, result) => {
    ...
    if (result.length === 1) {
      ...
      // PART 2: STATELESS AUTHENTICATION (Verifying and Token Generation)
      // Login Route
      // Centralized success handler to generate the JWT token
      const handleSuccess = () => {
  ...
  if (isHashed) {
        bcrypt.compare(password, user.password, (compareErr, isMatch) => {
          // Asynchronously compare the plaintext login password against the stored bcrypt hash
...
router.post("/users", verifyToken, requireRole(['admin']), (req, res) => {
  ...
  // PART 1: CRYPTOGRAPHIC SECURITY (Hashing new passwords)
  ...
  bcrypt.hash(defaultPassword, 10, (hashErr, hashedPassword) => {
    ...
    // The plaintext password is NEVER stored. Only the 'hashedPassword' is inserted.
...
// Allow a user to change their password (enforces strong password rules)
...
// Initiate the "Forgot Password" flow by generating a secure token
router.post("/forgot-password", (req, res) => {
  ...
  db.query(checkUserSql, [email], (err, result) => {
    ...
    // SECURITY TECHNIQUE: Anti-Enumeration.
    // Return success anyway to prevent hackers from fishing for valid registered emails.
    if (result.length === 0) {
      return res.json({ success: true, message: "If the email exists, a reset link was sent." });
    }

    // Generate a cryptographically secure random token and expiration (1 hour limit)
...
// Finalize password reset by verifying the token and applying the new hash
...
// Fetch facilities based on user role visibility
...
// Admin creates a new facility record
...
// Admin updates an existing facility
...
// Admin deletes a facility (Restricted if bookings exist)
...
// ============================================================================
// 4. AVAILABLE SLOTS & SCHEDULING LOGIC
// ============================================================================

// Calculate and return available time slots for a specific date and facility
/* ===== AVAILABLE 1-HOUR TIME SLOTS ===== */
router.get("/facilities/:id/available-slots", verifyToken, (req, res) => {
  const facilityId = req.params.id;
  const selectedDate = req.query.date;

  // Clean up any expired cubicles first to free up slots
  updateCubicleBookingStatuses(() => {
    ...
    // Retrieve facility operating hours and capacity
    const facilitySql = `
    ...
    db.query(facilitySql, [facilityId], (facilityErr, facilityResult) => {
      ...
      // Check existing bookings to calculate capacity limits
      const bookingSql = `
    ...
    db.query(bookingSql, [facilityId, selectedDate], (bookingErr, bookingResult) => {
        ...
        db.query(timetableSql, [facilityId, selectedDay], (timetableErr, timetableResult) => {
         ...
         // Facilities that use specific predefined manual slots instead of general 1-hour increments
...
// Automated Expiry Function
// Background interceptor function triggered before checking facility availability
function releaseExpiredCubicleBookings(callback) {

  // Targeted SQL UPDATE statement utilizing database-level time evaluation
  const sql = `
    UPDATE bookings b
  ...
  // Execute query asynchronously and return control via callback
...
router.post("/bookings", verifyToken, (req, res) => {
  ...
  db.getConnection((connErr, connection) => {
    ...
    // 1. Begin Atomic Transaction: Ensures ACID properties. All queries succeed or all fail.
    connection.beginTransaction((transactionErr) => {
      if (transactionErr) { connection.release(); return res.json({ success: false, message: "Failed to start transaction" }); }

      // 2. Locks this specific facility row to prevent concurrent booking requests from reading the capacity until this transaction finishes.
        ...
        connection.query(checkSql, [facility_id, booking_date, start_time, end_time], (checkErr, checkResult) => {
          ...
          connection.query(
            ...
            (insertErr, insertResult) => {
              ...
              //4a. Rollback: If insertion fails, revert all changes and release the lock. Guarantees the database never enters an inconsistent stat
              if (insertErr) {
                return connection.rollback(() => { connection.release(); res.json({ success: false, message: "Booking failed", error: insertErr.message }); });
              }

              // 4b. Commit: If successful, permanently save the changes.
